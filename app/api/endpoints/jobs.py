...
jobspy_scraper = JobSpyScraperService()
...
async def scrape_jobs_with_jobspy(
    user_id: str,
    results_wanted: int = Query(50, ge=10, le=200),
    db = Depends(get_async_db)
):
    """
    Scrape jobs using JobSpy library (Indeed, Google Jobs)
    Legal and ToS-compliant scraping
    """
    try:
        logger.info("JobSpy scrape request", user_id=user_id, results_wanted=results_wanted)

        # Get user preferences
        user_settings = await db.user_settings.find_one({"user_id": user_id})
        onboarding = await db.users.find_one({"_id": user_id})

        if not user_settings or not onboarding:
            raise HTTPException(status_code=404, detail="User preferences not found")

        # Build preferences for scraping
        preferences = {
            "desired_positions": onboarding.get("job_preferences", {}).get("desired_positions", ["software engineer"]),
            "preferred_locations": onboarding.get("job_preferences", {}).get("preferred_locations", ["USA"]),
            "employment_types": onboarding.get("job_preferences", {}).get("employment_types", ["full_time"]),
            "remote_preference": onboarding.get("job_preferences", {}).get("remote_preference", "any")
        }

        # Scrape jobs
        result = await jobspy_scraper.scrape_and_save_to_db(
            db=db,
        ...
        if result["success"]:
            return {
                "success": True,
                "message": f"Scraped {result['total_scraped']} jobs using JobSpy",
                "stats": {
                    "total_scraped": result['total_scraped'],
                    "new_jobs": result['inserted'],
        ...
        logger.error("JobSpy scrape error", error=str(e))
        raise HTTPException(status_code=500, detail=str(e))
