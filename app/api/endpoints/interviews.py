...
async def conduct_interview(
    ...
    try:
        ...
        if interview_id:
            # Get existing interview from MongoDB
        ...
        if request.userMessage:
            ...
            if "messages" not in interview:
                interview["messages"] = []
            interview["messages"].append(message)

        # Get conversation history
        history = interview.get("messages", [])

        # Parse workflow from database
        workflow_data = interview["workflow"] if isinstance(interview["workflow"], dict) else json.loads(interview["workflow"])
        ...
        if completion_idx != -1:
            ...
            if json_start != -1 and json_end != -1 and json_end > json_start:
                ...
                if evaluation:
                    ...
                    # Save evaluation in interview document
                    await db.interviews.update_one(
                        {"_id": interview["_id"]},
                        {
                            "$set": {
                                "evaluation": evaluation,
                                "evaluation_created_at": datetime.utcnow()
                            }
                        }
                    )
...
async def smart_conversation(
    ...
    try:
        # Permission check removed - all authenticated users can use smart conversation
        # This is a core feature for interview practice
...
class InterviewCreateRequestV2(BaseModel):
    role: str
    company: str
...
async def create_interview_session(
    request: InterviewCreateRequestV2,
    current_user: Dict[str, Any] = Depends(get_current_user),
    db = Depends(get_async_db)
):
    ...
    try:
        ...
        workflow = {
            ...
            "skills_focus": request.skills_focus,
        ...
        return {
            ...
            "interview": {
                ...
                "updated_at": interview_doc["updated_at"].isoformat()
            },
